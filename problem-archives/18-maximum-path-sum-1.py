...
def squareTriangle(data):
    maxLength = getMaxLength(data)
    _data = []

    for row in data:
        _data.append(row + [0] * (maxLength - len(row)))

    return _data


def getMinSumPath(matrix, dstPos):
    row, col = dstPos
...
if __name__ == '__main__':
    ...
    data = [[int(col) for col in row.split()] for row in data]

    # print(squareTriangle(data))
